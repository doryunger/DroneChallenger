...
## Two-page dialog flow: mission briefing, then a BT viewer intro

`EMenuState` has a third value, `BTInfo`, between `Dialog` (the existing mission briefing) and actually starting the game. Clicking the mission briefing's "CONTINUE" button no longer dismisses the menu directly — it advances `State` to `BTInfo`, which `DrawBTInfoScreen` renders using the exact same box/border/header layout code as `DrawDialogScreen` (same `DW`/`DH`/`DX`/`DY`/`Bdr` proportions, same gold-trim box, same pixel-font header style), just with different content: header "BT DISPLAY", a short real-font (not pixel-font) explanation of what the live behavior-tree viewer shows and where to find it in the HUD, and a button labeled "LET'S GO" that triggers the actual dismiss-and-launch (`bDismissing = true`) previously owned by "CONTINUE". `CtaMin`/`CtaMax` are reused across both pages rather than duplicated, since only one page is ever drawn/hit-tested at a time.

The pixel-font glyph set (`GetPixelGlyph`) originally only covered capital letters, no punctuation, so an apostrophe would silently render as a blank same-width gap rather than an error. Added `GlyphApostrophe` — a compact two-pixel flick confined to the top two rows of the 7-row cell (unlike the full-height letter glyphs), since an apostrophe should sit high rather than spanning the whole cell — so "LET'S GO" now renders with an actual mark instead of a gap. The body paragraph explaining the viewer uses real text (`FSlateDrawElement::MakeText`), not the pixel-glyph system, so normal punctuation there was never affected.

`NativeOnMouseButtonDown` previously didn't check *which* mouse button triggered a CTA click at all — any button (left, right, middle) would fire it. Added an explicit `InMouseEvent.GetEffectingButton() != EKeys::LeftMouseButton` early-out (after the Title→Dialog transition, which deliberately stays any-button/any-key, matching its "press anything to start" prompt) so both the mission-briefing and BT-info buttons only respond to a genuine left click.
