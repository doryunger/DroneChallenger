...
`EMenuState` has a third value, `BTInfo`, between `Dialog` (the existing mission briefing) and actually starting the game. Clicking the mission briefing's "CONTINUE" button no longer dismisses the menu directly — it advances `State` to `BTInfo`, which `DrawBTInfoScreen` renders using the exact same box/border/header layout code as `DrawDialogScreen` (same `DW`/`DH`/`DX`/`DY`/`Bdr` proportions, same gold-trim box, same pixel-font header style), just with different content: header "BT DISPLAY", a short real-font (not pixel-font) explanation of what the live behavior-tree viewer shows and where to find it in the HUD, and a button labeled "LETS GO" that triggers the actual dismiss-and-launch (`bDismissing = true`) previously owned by "CONTINUE". `CtaMin`/`CtaMax` are reused across both pages rather than duplicated, since only one page is ever drawn/hit-tested at a time.

The button label is "LETS GO" without the apostrophe, not a typo: the pixel-font glyph set (`GetPixelGlyph`) only covers capital letters, no punctuation, so an apostrophe would silently render as a blank same-width gap rather than an error — readable either way, but dropping it keeps the retro-pixel label clean. The body paragraph explaining the viewer uses real text (`FSlateDrawElement::MakeText`), not the pixel-glyph system, so normal punctuation there is unaffected.
