	...
	float OutThrottle[4])
{
	...
	const float TiltAngleDeg  = FMath::RadiansToDegrees(FMath::Acos(FMath::Clamp(ActorUp.Z, 0.f, 1.f)));
	...
	const bool bRollCorrects  = (FMath::Abs(BodyAngVelDeg.X) > 2.f && Input.Roll  * BodyAngVelDeg.X < 0.f)
	                          || (FMath::Abs(TiltRollRad)  > 0.035f && Input.Roll  * TiltRollRad  < 0.f);
	const bool bPitchCorrects = (FMath::Abs(BodyAngVelDeg.Y) > 2.f && Input.Pitch * BodyAngVelDeg.Y < 0.f)
